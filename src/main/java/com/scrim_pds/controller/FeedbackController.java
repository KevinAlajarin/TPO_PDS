...
public class FeedbackController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista de feedback aprobado",
                         content = @Content(mediaType = "application/json", schema = @Schema(type = "array", implementation = Feedback.class))),
            @ApiResponse(responseCode = "404", description = "Scrim no encontrado")
    })
    @GetMapping("/api/scrims/{id}/feedback")
    public ResponseEntity<List<Feedback>> getFeedbackForScrim(
            @Parameter(description = "ID del Scrim") @PathVariable("id") UUID scrimId
    ) throws IOException {
        // (Nota: Este endpoint es público, cualquiera puede ver feedback APROBADO)
        List<Feedback> feedbackList = feedbackService.getApprovedFeedbackForScrim(scrimId);
        return ResponseEntity.ok(feedbackList);
    }

    // --- Endpoints de Administración (Moderación) ---
    // (Implementación básica de seguridad de roles)
