...
public class ScrimController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista de scrims encontrados",
                         ...
                         schema = @Schema(type = "array", implementation = Scrim.class))) // Indica que devuelve una lista de Scrims
    })
    @GetMapping
    public ResponseEntity<List<Scrim>> getScrims(
            @Parameter(description = "Filtrar por nombre exacto del juego (ej. Valorant)", required = false) @RequestParam Optional<String> juego,
            @Parameter(description = "Filtrar por región exacta (ej. LATAM)", required = false) @RequestParam Optional<String> region,
            @Parameter(description = "Filtrar por rango mínimo exacto (ej. Oro)", required = false) @RequestParam Optional<String> rangoMin,
            @Parameter(description = "Filtrar por rango máximo exacto (ej. Platino)", required = false) @RequestParam Optional<String> rangoMax,
            @Parameter(description = "Filtrar por latencia máxima permitida (ej. 100)", required = false) @RequestParam Optional<Integer> latenciaMax
    ) throws IOException {
        List<Scrim> scrims = scrimService.findScrims(juego, region, rangoMin, rangoMax, latenciaMax);
        return ResponseEntity.ok(scrims);
    }

    @Operation(summary = "Crear un nuevo scrim", security = @SecurityRequirement(name = "bearerAuth")) // Indica que requiere el token
    @ApiResponses(value = {
    ...
    public ResponseEntity<Void> subirEstadisticas(
            ...
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Lista de estadísticas por jugador") // Mejor descripción del body
